...
def write_prevalence_tex(prev_df: pd.DataFrame) -> None:
    lines = []
    lines.append("% Auto-generated by python/22_summarize_construction_battery.py")
    lines.append("\\begin{longtable}{l l r}")
    ...
    lines.append("  \\toprule")
    lines.append("  Construction & Corpus & Candidates (positives) \\\\")
    lines.append("  \\midrule")
    ...
    lines.append("  \\midrule")
    lines.append("  \\multicolumn{3}{r}{\\emph{Continued on next page}} \\\\")
    lines.append("  \\endfoot")
    lines.append("  \\bottomrule")
    lines.append("  \\endlastfoot")
    ordered = prev_df.sort_values(["construction", "corpus"])
    for _, row in ordered.iterrows():
        construction_tex = latex_escape(str(row["construction"]))
        corpus = str(row["corpus"]).upper()
        n = int(row["n_candidates"])
        p = int(row["n_positive"])
        lines.append(f"  {construction_tex} & {corpus} & {n}({p}) \\\\")
