#!/usr/bin/env python3
"""
Compute precision-recall curves and evaluation metrics for the or even cue bundle.

Outputs:
  - out/or_even_eval.csv
  - images/or_even_prcurve.pdf (GUM->EWT)
  - images/appendix/or_even_prcurve_ewt2gum.png (EWT->GUM)
  - out/or_even_errors.tsv
"""
...
IN_PATH = os.path.join("out", "or_even_anchor_present_eval.csv")
OUT_EVAL = os.path.join("out", "or_even_eval.csv")
OUT_ERRS = os.path.join("out", "or_even_errors.tsv")
...
def main() -> None:
    ...
    if curves:
        plot_curves(curves, os.path.join("images", "or_even_prcurve.pdf"), "PR curves: GUM -> EWT")
    ...
    for name, feats in FEATURE_SETS.items():
        ...
        if metrics["rec_curve"].size and metrics["prec_curve"].size:
            curves[name] = (metrics["rec_curve"], metrics["prec_curve"])
    if curves:
        plot_curves(curves, os.path.join("images", "appendix", "or_even_prcurve_ewt2gum.png"), "PR curves: EWT -> GUM")
    ...
    # errors for GUM->EWT full model if estimable
    if len(np.unique(gum["label"])) >= 2:
        probs, _ = train_predict(gum, ewt, FEATURE_SETS["full"])
        errors = ewt.copy()
