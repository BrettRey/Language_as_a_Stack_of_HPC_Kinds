...
def main() -> None:
    ...
    # Add canonical label: anchor present + parallelism + licensing
    features['is_canonical'] = (features['anchor_present'].astype(bool)) & (features['parallelism'].astype(bool)) & (features['licensing'].astype(bool))
    # Anchor-present evaluation set (positives: is_canonical; negatives: anchor-present but failing criteria)
    anchor_eval = features[features['anchor_present']].copy()
    anchor_eval['label'] = anchor_eval['is_canonical'].astype(int)
    apath = os.path.join("out", "let_alone_anchor_present_eval.csv")
    anchor_eval.to_csv(apath, index=False)
    print(f"Saved anchor-present evaluation set to {apath}")
