...
To reduce tautological saturation, instance labels are defined by a stricter heuristic than the three cue proxies, so cues predict rather than define labels. The heuristics were also broadened to target structural signatures (copular all-clefts, paired \mention{the} clauses, possessive+\mention{way} with path framing). Under this regime, six constructions meet the minimum thresholds (Table~\ref{tab:cx-battery-prevalence}): N--P--N, resultative\_adj, resultative\_pooled, all-clefts, comparative correlatives, and the \mention{way} construction. The expanded corpus palette yields 36 estimable train--test pairs for these constructions (notably across CHILDES and LinES in addition to GUM/EWT). Table~\ref{tab:cx-battery-results} summarizes the full-model PR--AUC and ablation deltas for those pairs. Full PR--AUC now ranges from 0.31 to 1.00; Cue~3 ablations still produce the largest drops overall, but all-clefts show moderate sensitivity to Cue~1 and the \mention{way} construction shows strong sensitivity to Cue~1/Cue~2.

By the preregistered decision rule, N--P--N and the \mention{way} construction count as \emph{provisional positives at this grain}. All-clefts are borderline (two train--test directions exceed the 0.70 target; others do not), comparative correlatives fail on PR--AUC despite meeting prevalence thresholds, and the resultative outcomes remain \emph{brakes}: both the pooled set and the AP subtype depend almost entirely on Cue~3, consistent with the \enquote{too fat} prediction, while the PP/path subtype is too sparse to evaluate. Binominal \mention{N of a N} and the remaining targets (\mention{or even}, \mention{time-away}, \mention{just because} \dots \mention{doesn't mean} \dots, \mention{X much?}) still fall below prevalence thresholds, so no kind claim is made for them in this run.
...
The homeostasis diagnostics temper the story further. Cue covariance (Table~\ref{tab:cx-battery-cov}) is often undefined because one cue is invariant across positives, and where it is defined the remaining cue pair tends to show modest or negative association (e.g., \mention{or even}). This points to cue specialization or trade-offs rather than robust redundancy in the current slices. Downsampling the training data to 25\% produces only small changes in PR--AUC (max $|\Delta| \approx 0.08$) across the 36 estimable pairs (Table~\ref{tab:cx-battery-downsample}), suggesting that even after loosening the heuristics the cue bundle still yields stable separability on these candidate sets. As a result, the perturbation test remains weakly informative at this grain and likely requires broader samples or less restrictive label heuristics to become diagnostic. Bootstrap CIs (Table~\ref{tab:cx-battery-bootstrap}) reinforce the pattern: N--P--N and \mention{way} remain entirely above the 0.70 target, all-clefts straddle the threshold, and comparative correlatives remain below it. Shuffled-label baselines (Table~\ref{tab:cx-battery-shuffle}) track prevalence—especially for the high-prevalence N--P--N pairs—and for comparative correlatives the full model fails to beat the shuffle mean, strengthening the negative result.
