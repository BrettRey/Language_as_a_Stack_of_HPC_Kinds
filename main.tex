...
\subsection{Construction battery prevalence and results}

This run yields five constructions with estimable cross-corpus pairs under the minimum thresholds (Table~\ref{tab:cx-battery-prevalence}). Table~\ref{tab:cx-battery-results} summarizes the full-model PR--AUC and ablation deltas for those pairs. Constructions failing thresholds are retained as prevalence-only evidence, since zeros and sparsity delimit their distributional scope rather than registering as failures.
