...
\textbf{Projectibility.} Prototype classifier trained on 1900--1940 and tested on 1950--2000. Metrics: macro-F1 across all lexemes and per-lexeme accuracy (targets vs controls). Shuffled-label baseline uses 200 permutations. Outputs: \texttt{out/word\_drift\_summary.csv}, \texttt{out/word\_drift\_lexemes.csv}, \texttt{out/word\_drift\_shuffle.csv}.
...
\textbf{Corpus palette.} Construction-battery analyses use the UD English palette (GUM, EWT, GUMReddit, ESL, ESLSpok, GENTLE, CHILDES, LinES, PUD, ParTUT, UniDive, ATIS, LittlePrince, Pronouns, CTeTex, PCEDT). Register is treated as an explicit experimental factor via the three-step protocol in \S\ref{sec:cx-candidates}.
