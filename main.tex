...
To reduce tautological saturation, instance labels are defined by a stricter heuristic than the three cue proxies, so cues now predict rather than define labels. Under this regime, three constructions meet the minimum thresholds (Table~\ref{tab:cx-battery-prevalence}): N--P--N, resultative\_adj, and resultative\_pooled. The expanded corpus palette yields 26 estimable train--test pairs for these constructions (notably across CHILDES and LinES in addition to GUM/EWT). Table~\ref{tab:cx-battery-results} summarizes the full-model PR--AUC and ablation deltas for those pairs. Full PR--AUC now ranges from 0.68 to 1.00, with Cue~3 ablations producing the largest drops ($\Delta=0.17$ to $0.84$). Cue~1 and Cue~2 ablations do not reduce PR--AUC in these slices, and the remaining constructions fall below the prevalence thresholds under the stricter labeling.

By the preregistered decision rule, only N--P--N counts as a \emph{provisional positive at this grain}. The resultative outcomes remain \emph{brakes}: both the pooled set and the AP subtype depend almost entirely on Cue~3, consistent with the \enquote{too fat} prediction, while the PP/path subtype is too sparse to evaluate. All-clefts, binominal \mention{N of a N}, comparative correlatives, and the remaining targets (\mention{or even}, \mention{way}, \mention{time-away}, \mention{just because} \dots \mention{doesn't mean} \dots, \mention{X much?}) fall below prevalence thresholds under the stricter labeling, so no kind claim is made for them in this run.

The implication is deliberately narrow: the construction tier does not license a blanket \textsc{HPC} claim. Kindhood is earned case-by-case, and at this grain it attaches only to N--P--N, while the resultatives behave as a failure mode that supports the \enquote{too fat} diagnosis. The dominance of Cue~3 in the ablations further suggests that, in these slices, one cue can do most of the work; stronger evidence for multi-cue redundancy will require larger samples and the planned downsampling and covariance checks.

The homeostasis diagnostics temper the story further. Cue covariance (Table~\ref{tab:cx-battery-cov}) is often undefined because one cue is invariant across positives, and where it is defined the remaining cue pair tends to show modest or negative association (e.g., \mention{or even}). This points to cue specialization or trade-offs rather than robust redundancy in the current slices. Downsampling the training data to 25\% produces effectively zero changes in PR--AUC across the 26 estimable pairs (Table~\ref{tab:cx-battery-downsample}), suggesting that even after loosening the heuristics the cue bundle still yields stable separability on these candidate sets. As a result, the perturbation test remains weakly informative at this grain and likely requires broader samples or less restrictive label heuristics to become diagnostic.
