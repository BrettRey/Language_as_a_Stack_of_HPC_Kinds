...
The homeostasis diagnostics temper the story further. Cue covariance (Table~\ref{tab:cx-battery-cov}) is often undefined because one cue is invariant across positives, and where it is defined the remaining cue pair tends to show modest or negative association (e.g., binominal \mention{N of a N}, \mention{or even}). This points to cue specialization or trade-offs rather than robust redundancy in the current slices. Downsampling the training data to 25\% produces negligible changes in PR--AUC (Table~\ref{tab:cx-battery-downsample}), largely because the candidate heuristics already yield saturated separability. As a result, the perturbation test is inconclusive at this grain and requires broader samples or weaker heuristics to become diagnostic.
